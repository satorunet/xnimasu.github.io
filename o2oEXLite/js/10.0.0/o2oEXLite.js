...
function crUI() {
	...
	function crMarkerUI() {
		...
		mrkCorrectBox.append(hr(), mrkInCorrectCheck, mrkInCorrectLengthSelect, br(), mrkOutCorrectCheck, mrkOutCorrectLengthSelect);
...
//		var pathTypeTextList = ['直線', 'スプライン曲線', '2次ペジェ曲線', '3次ペジェ曲線'];
		...
		var pathTypeSelect = crSelectEl('pathTypeSelect', '種類：', pathTypeValueList, pathTypeTextList, 0).change(changeToolAction);
		...
		var pathMenu = $('<div>', {id: 'pathMenu'}).append(pathTypeSelect, pathAnchorSizeSelect, pathSplineInterpolateSelect, hr(), pathCommitBtn);
...
function clickMenu(tool) {
	console.log(sketch.tool);
...
function startPath(e) {
	...
	switch (canvas.path.mode) {
		...
		case PATH_MODE.NONE:
			canvas.path.mode = PATH_MODE.MOVE;
			
			var i = checkPoint(canvas.previewCtx, p, canvas.action.path.anchors, 'red');
			if (i < 0) {
				canvas.action.path.anchors.push(p);
				canvas.path.mp = canvas.action.path.anchors.length-1;
			} else {
				canvas.path.mp = i;
			}
			break;
...
function stopPath() {
	switch (canvas.path.mode) {
		case PATH_MODE.MOVE:
			canvas.path.mode = PATH_MODE.NONE;
			var i = checkPoint(canvas.previewCtx, canvas.path.lp, canvas.action.path.anchors, 'red');
			if (i >= 0) canvas.action.path.anchors[canvas.path.mp] = canvas.action.path.anchors[i];
			canvas.path.mp = -1;
			break;
	}
}
...
function commitPath() {
	if (canvas.path.mode != PATH_MODE.INIT) {
		canvas.path.mode = PATH_MODE.INIT;
...
function previewPath(e) {
	if (canvas.path.mode == PATH_MODE.INIT) return;
	
	var p = e ? getPosition(e) : {x: 0, y: 0};
	
	var anchors = canvas.action.path.anchors;
	switch (canvas.action.path.type) {
		case 'line':
			canvas.action.events = anchors;
			break;
		case 'spline':
			if (anchors.length > 1) {
				canvas.action.events = splineStream(anchors, canvas.path.interpolate);
			} else {
				canvas.action.events = anchors;
			}
			break;
	...
	switch (canvas.path.mode) {
		case PATH_MODE.MOVE:
			canvas.action.path.anchors[canvas.path.mp] = p;
			clearCanvas(canvas.previewCtx);
			drawPath(canvas.previewCtx, canvas.action);
			drawAnchors(canvas.previewCtx, canvas.action.path.anchors);
			checkPoint(canvas.previewCtx, p, canvas.action.path.anchors, 'red');
			break;
		case PATH_MODE.NONE:
			clearCanvas(canvas.previewCtx);
			drawPath(canvas.previewCtx, canvas.action);
...
function drawAnchors(ctx, anchors) {
	var r = $('#pathAnchorSizeSelect').val();
