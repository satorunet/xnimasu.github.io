...
function crUI() {
	...
	function crMarkerUI() {
		...
		var markerMenu = $('<div>', {id: 'markerMenu'}).append(mrkTypeSelect, mrkCapSelect, alphaLockCheck, br(), mrkCorrectBox, mrkPatBox);
...
function stopPath() {
	switch (canvas.path.state) {
		...
		case PATH_STATE.DRAW:
			...
			canvas.previewStart = '';
			canvas.action.path.ap = extractAnchor(canvas.action.path.op);
			canvas.path.state = PATH_STATE.NONE;
	}
}

function extractAnchor(op) {
	var extract = canvas.path.extract;
	var ap = [];
	for (var i in op) {
		if(i%extract == 0) {
			ap.push(op[i]);
