...
function crUI() {
	...
	function crLayerUI() {
		...
		layerAlphaBox.append(layerAlphaText, layerAlphaSlider);
...
function setHighModeEvent() {
	...
	previewMarker = function(e){
		...
		canvas.fix.timer = setTimeout(function() {
			marking(p.x, p.y);
		}, FIX_INTERVAL);
		
		function marking(x, y) {
			clearTimeout(canvas.fix.timer);
			
			var fp = getFixPosition(x, y);
			canvas.action.events.push({x: fp.x, y: fp.y});
...
function showPointer(e) {
	...
	function show(x, y) {
		...
		canvas.inputCtx.arc(fp.x, fp.y, canvas.pointer.w/2, 0, Math.PI*2, false);
...
function selectLayer(num) {
	...
	//$('#layerAlphaSlider').slider({value: canvas.layer.ctx.globalAlpha*100});
	//changeLayerAlpha(canvas.layer.ctx.globalAlpha*100);
...
function startFreePath(ctx, action) {
	//clearCanvas(ctx);
	//ctx.drawImage(canvas.layer.el[0], 0, 0);
...
function getFixPosition(x, y) {
	if (x > canvas.fix.x) {
		x -= (x-canvas.fix.x) * canvas.fix.lv / 10;
	} else {
		x += (canvas.fix.x-x) * canvas.fix.lv / 10;
	}

	if (y > canvas.fix.y) {
		y -= (y-canvas.fix.y) * canvas.fix.lv / 10;
	} else {
		y += (canvas.fix.y-y) * canvas.fix.lv / 10;
	}
