...
var ls = {
	...
	hotkey: loadStorage(LS_KEY.HOTKEY, true) || DEF_HOTKEY,
}
...
//		var pathTypeTextList = ['直線', 'スプライン曲線', '2次ペジェ曲線', '3次ペジェ曲線'];
  ...
  function crFillUI() {
	...
	function crSettingsUI() {
		...
		var tabName = 'tab-settings-';	
		
		var modeMenu = $('<div>', {id: tabName +'modeMenu'});
		...
		$('body').append(modeMenu.append(modeSelect));

		var dispMenu = $('<div>', {id: tabName +'dispMenu'});
		if (PC) {
		...
		if(PC) $('body').append(dispMenu.append(guideCheck));

		var hotkeyMenu = $('<div>', {id: tabName +'hotkeyMenu'});
		var hotkeyTbl = $('<table>', {id: 'hotkeyTbl'});
		for (var key in ls.hotkey) {
			var value = ls.hotkey[key];
			$('<tr>').append($('<td>', {text: value}));
		}
		
		var subMenu = $('<div>', {id: 'settingsSubMenu'}).append($('<ul>'));
		var subMenuValueList = ['mode', 'disp', 'hotkey'];
		var subMenuTextList = ['モード', '表示', 'キー設定'];
		for (var i in subMenuValueList) {
			var value = subMenuValueList[i];
			var text = subMenuTextList[i];
			crSubMenu(subMenu, value, text);
		}
		
		var settingsMenu = $('<div>', {id: 'settingsMenu'}).append(subMenu.tabs({active: 0}));
		$('body').append(settingsMenu);
		
		function crSubMenu(menu, value, text) {
			var li = $('<li>').append($('<a>', {
				href: '#' + tabName + value + 'Menu',
				text: text,
				click: function() {
					//
				}
			...
			menu.append($('#'+tabName+value+'Menu'));
		}

	}
	
	function crHotkeySel() {
...
function changeTool(beforeTool) {
	$('a[href=#tab-' + beforeTool + ']').trigger('click');
}
...
function drawSelPaste(ctx, action) {
	...
	ctx.drawImage(canvas.tmpEl[0], 0, 0, selArea.w, selArea.h, action.events[2].x, action.events[2].y, selArea.w, selArea.h);
}
...
function setKeyDownEvent() {
	...
	$(document).keydown(function(e){
		for (var i in textInputTypes) {
			if(textInputTypes[i] == e.target.nodeName) return;
		}
		
		changeActionList(e, 90, canvas.layer.actions, redoList);
		changeActionList(e, 89, redoList, canvas.layer.actions);
		
		var key = String.fromCharCode(e.which).toUpperCase();
		var tool = ls.hotkey[key];
		if (tool) changeTool(tool);
	});
}

function changeActionList(e, keyNum, popList, pushList){
