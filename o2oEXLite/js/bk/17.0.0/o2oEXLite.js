...
function crLayer() {
	...
	setCanvasSize(layer[0], canvas.w, canvas.h, canvas.showScale);
	crLayerPreview(num);
...
function startDraw(ctx, action) {
	var grad;
	if (fool) {
		var colors = ['#F66', '#F6F', '#66F', '#6F6', '#FF6', '#FA6', 
									'#F66', '#F6F', '#66F', '#6F6', '#FF6', '#FA6', 
									'#F66', '#F6F', '#66F', '#6F6', '#FF6', '#FA6',
								 ,'#F66', '#F6F', '#66F', '#6F6', '#FF6', '#FA6'];
...
function getPosition(e) {
	...
	var offsetX = offset.left;
	var offsetY = offset.top;
	var x = (e.pageX - offsetX) / canvas.showScale;
	var y = (e.pageY - offsetY) / canvas.showScale;
...
function changeCanvasSize() {
	...
	canvas.w = size.w;
	canvas.h = size.h;
	canvas.showScale = 1//$('#scaleSelect').val() || 1;
	canvas.scale = $('#scaleSelect').val() || 1;

	setCanvasSize(sketch.el, size.w, size.h, canvas.showScale, canvas.scale);
	setCanvasSize(canvas.inputEl[0], size.w, size.h, canvas.showScale, canvas.scale);
	setCanvasSize(canvas.tmpEl[0], size.w, size.h, canvas.showScale, canvas.scale);
	setCanvasSize(canvas.previewEl[0], size.w, size.h, canvas.showScale, canvas.scale);
	setPictCrop();

	var layerNum = canvas.layerNum;
	for (var i in canvas.layers) {
		var layer = canvas.layers[i];
		setCanvasSize(layer.el[0], size.w, size.h, canvas.showScale, canvas.scale);
		setCanvasSize(layer.preview.el[0], size.w, size.h, PREVIEW_SIZE, canvas.scale);
		selectLayer(i);
	}
	selectLayer(layerNum);
	redraw();
...
function setCanvasSize(el, w, h, showScale, canvasScale) {
	canvasScale = canvasScale || 1;
	el.width = w;
	el.height = h;
	el.style.width = w * canvasScale * showScale;
	el.style.height = h * canvasScale * showScale;
	el.width = w * canvasScale;
	el.height = h * canvasScale;
}
